import { c as w, e as b } from "./es6.BJcsVXQ0.js";
import "./index.CAj-7vWz.js";
const { WritableStream: m, TransformStream: y, DOMException: P, Blob: _ } = w,
  { GONE: v } = b,
  E = /constructor/i.test(window.HTMLElement);
class C {
    ...
    if (((o.download = this.name), E || !t)) {
      ...
      n.readable.pipeTo(
        new m({
          ...
          close() {
            ...
            ((r = []),
              (o.href = URL.createObjectURL(s)),
              o.click(),
              setTimeout(() => URL.revokeObjectURL(o.href), 1e4));
          },
        }),
      );
    } else {
      const { writable: r, readablePort: s } = new S(m),
        a = encodeURIComponent(this.name)
          .replace(/['()]/g, escape)
          .replace(/\*/g, "%2A"),
        R = {
      ...
      (n.readable
        .pipeThrough(
          new y({
            transform(c, d) {
              ...
              const g = new Response(c).body.getReader(),
                h = (j) =>
                  g.read().then((p) => (p.done ? 0 : h(d.enqueue(p.value))));
              return h();
            },
          }),
        )
        ...
        t.active.postMessage(
          { url: t.scope + a, headers: R, readablePort: s },
          [s],
        ));
...
class k {
  constructor(e) {
    ((e.onmessage = (t) => this._onMessage(t.data)),
      (this._port = e),
      this._resetReady());
  }
  start(e) {
  ...
  _onMessage(e) {
    (e.type === T && this._resolveReady(),
      e.type === L && this._onError(e.reason));
  }
  _onError(e) {
...
class S {
  constructor(e) {
    ...
    ((this.readablePort = t.port1), (this.writable = new e(new k(t.port2))));
  }
}
export { C as FileHandle };
